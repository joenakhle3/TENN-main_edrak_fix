...
from tenn_ai.fabric_ai.generate_ai.camel.agents import RolePlaying
from tenn_ai.fabric_ai.generate_ai.camel.messages import ChatMessage
from tenn_ai.fabric_ai.generate_ai.camel.typing import TaskType, ModelType
from tenn_ai.fabric_ai.generate_ai.chatdev.chat_env import ChatEnv
from tenn_ai.fabric_ai.generate_ai.chatdev.statistics import get_info
from tenn_ai.fabric_ai.generate_ai.chatdev.utils import log_and_print_online, log_arguments
...
class InputAnalysis(Phase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)

    # This code updates the phase conversation before the phase starts, by setting any needed variables from the environment
    # You can use this phase to pre-process anything you need to do before the phase starts
    def update_phase_env(self, chat_env):
        # ADD CODE HERE
        pass # DELETE THIS

    # This code updates the environment and performs activities based on the final conclusion of the conversation in a phase
    # You can use this phase to post-process anything you need to do after the phase ends
    def update_chat_env(self, chat_env) -> ChatEnv:
        if len(self.seminar_conclusion) > 0:
            chat_env.env_dict['input'] = self.seminar_conclusion.split("<INFO>")[-1].lower().replace(".", "").strip()
        return chat_env
